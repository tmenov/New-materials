...
Полная версия со встроенными данными всех 118 элементов
"""
...
ELEMENTS_DATA = [
    # (Z, Symbol, Name, Mass, Electron_config, Most_common_isotope, Stability, Ionization_energy)
...
class HybridElement:
    ...
    def print_detailed_energy_report(self):
        ...
        for energy_type in EnergyType:
            ...
            print(f"   {energy_type.value:20}: {total_quanta:8,d} квантов ({percentage:6.2f}%)")
...
def main():
    ...
    print("Полная система из 118 элементов со встроенными данными")
    print("=" * 80)
    ...
    # Детальный отчет для ВСЕХ элементов в файл
    print(f"\n{'='*80}")
    print("СОХРАНЕНИЕ ДЕТАЛЬНЫХ ОТЧЕТОВ ДЛЯ ВСЕХ ЭЛЕМЕНТОВ В ФАЙЛ")
    print(f"{'='*80}")

    with open("полный_энергетический_анализ_fin-v5-6.txt", "w", encoding="utf-8") as f:
        for element in system.elements:
            f.write(f"\n{'='*80}\n")
            f.write(f"ДЕТАЛЬНЫЙ ЭНЕРГЕТИЧЕСКИЙ ОТЧЕТ: {element.name} ({element.symbol})\n")
            f.write(f"{'='*80}\n")
        
            f.write(f"Энергетическая сигнатура: {element.energy_signature}\n")
            f.write(f"Общее число квантов: {element.energy_profile.total_quanta:,d}\n")
            f.write(f"Резонансная частота: {element.energy_profile.resonance_frequency:.3e} Гц\n")
            f.write(f"Коэффициент вибрации: {element.energy_profile.vibrational_coefficient:.3f}\n")
        
            f.write(f"\nРАСПРЕДЕЛЕНИЕ КВАНТОВ ПО УРОВНЯМ:\n")
            for level, level_data in element.energy_profile.level_quanta.items():
                f.write(f"\n● {level.value}:\n")
                f.write(f"   Всего квантов: {level_data.total_quanta:8,d}\n")
                for energy_type, quanta in level_data.energy_distribution.items():
                    percentage = (quanta / level_data.total_quanta * 100) if level_data.total_quanta > 0 else 0
                    f.write(f"   {energy_type.value:20}: {quanta:8,d} квантов ({percentage:5.1f}%)\n")
        
            f.write(f"\nОБЩЕЕ РАСПРЕДЕЛЕНИЕ ПО ТИПАМ ЭНЕРГИЙ:\n")
            total_energy = element.energy_profile.total_quanta
            for energy_type in EnergyType:
                total_quanta = element.get_total_energy_quanta(energy_type)
                percentage = (total_quanta / total_energy * 100) if total_energy > 0 else 0
                f.write(f"   {energy_type.value:20}: {total_quanta:8,d} квантов ({percentage:5.1f}%)\n")

    print("✓ Полные отчеты по всем 118 элементам сохранены в файл 'полный_энергетический_анализ_fin-v5-6.txt'")
