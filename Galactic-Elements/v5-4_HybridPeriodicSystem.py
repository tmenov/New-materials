...
def main():
    ...
    # Детальный отчет для ВСЕХ элементов по группам
    print(f"\n{'='*80}")
    print("ДЕТАЛЬНЫЙ ЭНЕРГЕТИЧЕСКИЙ ОТЧЕТ ДЛЯ ВСЕХ ЭЛЕМЕНТОВ")
    print(f"{'='*80}")

    # Вывод по 10 элементов за раз с паузами
    for i, element in enumerate(system.elements, 1):
        element.print_detailed_energy_report()
        if i % 10 == 0:  # Пауза после каждых 10 элементов
