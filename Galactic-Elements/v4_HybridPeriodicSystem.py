...
Полная версия для всех 118 элементов с расчетом квантов по уровням
"""
...
class EnergyLevel(Enum):
    ...
    LEVEL_MICRO_G = "Уровень микрогравитации"
    LEVEL_MACRO_G = "Уровень макрогравитации"
    LEVEL_THERMONUCLEAR = "Термоядерный уровень"
    LEVEL_RADIOACTIVITY = "Радиоактивный уровень"
...
# Распределение энергий по уровням (согласно модели МС)
LEVEL_ENERGY_DISTRIBUTION = {
    EnergyLevel.LEVEL_3: 0.55,  # 55% - третий уровень
    EnergyLevel.LEVEL_2: 0.33,  # 33% - второй уровень
    EnergyLevel.LEVEL_1: 0.12   # 12% - первый уровень
}

# Распределение энергий на первом уровне (ступенчатая пирамида)
...
# Распределение энергий на втором уровне
...
# Распределение энергий на третьем уровне
...
class EnergyLevelQuanta:
    """Кванты энергии на конкретном уровне"""
    level: EnergyLevel
    total_quanta: int = 0
...
class QuantumEnergyProfile:
    """Полный энергетический профиль элемента"""
    total_quanta: int = 0
    level_quanta: Dict[EnergyLevel, EnergyLevelQuanta] = field(default_factory=dict)
    resonance_frequency: float = 0.0
    vibrational_coefficient: float = 0.0  # Коэффициент вибрационной активности
...
class HybridElement:
    ...
    energy_signature: str = ""  # Уникальная энергетическая сигнатура
    ...
    def _calculate_energy_profile(self):
        """Расчет полного энергетического профиля по модели МС"""
        base_quanta = 1600  # Базовое количество для водорода
        ...
        # Специальные уровни (рассчитываются отдельно)
        micro_g_quanta = int(self.nucleus.binding_energy * 800 * self.atomic_mass)
        macro_g_quanta = int(self.atomic_mass * 400 * (1 + self.nucleus.neutrons / max(1, self.nucleus.protons)))
        thermonuclear_quanta = int(self.nucleus.binding_energy * 1500 * self.nucleus.stability)
        radioactivity_quanta = int((1 - self.nucleus.stability) * 8000 * self.atomic_mass)
        
        # Добавляем специальные уровни
        self.energy_profile.level_quanta[EnergyLevel.LEVEL_MICRO_G] = EnergyLevelQuanta(
            EnergyLevel.LEVEL_MICRO_G, micro_g_quanta, {EnergyType.MICRO_GRAVITY: micro_g_quanta}
        )
        self.energy_profile.level_quanta[EnergyLevel.LEVEL_MACRO_G] = EnergyLevelQuanta(
            EnergyLevel.LEVEL_MACRO_G, macro_g_quanta, {EnergyType.MACRO_GRAVITY: macro_g_quanta}
        )
        self.energy_profile.level_quanta[EnergyLevel.LEVEL_THERMONUCLEAR] = EnergyLevelQuanta(
            EnergyLevel.LEVEL_THERMONUCLEAR, thermonuclear_quanta, {EnergyType.THERMONUCLEAR: thermonuclear_quanta}
        )
        self.energy_profile.level_quanta[EnergyLevel.LEVEL_RADIOACTIVITY] = EnergyLevelQuanta(
            EnergyLevel.LEVEL_RADIOACTIVITY, radioactivity_quanta, {EnergyType.RADIOACTIVITY: radioactivity_quanta}
        )
        
        # Резонансная частота и вибрационный коэффициент
        ...
        self.energy_profile.vibrational_coefficient = (
            ...
            (self.electrons.ionization_energy / 24.587) * 0.2  # Нормировка по гелию
        )
    ...
    def _generate_energy_signature(self):
        """Генерация уникальной энергетической сигнатуры"""
    ...
    def get_total_energy_quanta(self, energy_type: EnergyType) -> int:
        """Получить общее количество квантов для конкретного типа энергии"""
        total = 0
    ...
    def print_detailed_energy_report(self):
        """Детальный отчет по энергетическому профилю"""
        print(f"\n{'='*80}")
        ...
        for energy_type in EnergyType:
            ...
            percentage = (total_quanta / self.energy_profile.total_quanta * 100) if self.energy_profile.total_quanta > 0 else 0
...
class HybridPeriodicSystem:
    ...
    def analyze_energy_distribution(self):
        """Анализ общего распределения энергии в системе"""
        print(f"\n{'='*80}")
...
def main():
    ...
    print("С расчетом квантов энергии по уровням (модель Межзвездного Союза)")
    print("=" * 80)
    ...
    print(f"{'='*140}")
    print(f"{'Z':3} {'Elem':5} {'Name':12} {'Type':18} {'Total Q':10} {'Res Freq':12} {'Vib Coef':8} {'Signature':20}")
    print("-" * 140)
    
    for element in system.elements:
        print(f"{element.atomic_number:3d} {element.symbol:5} {element.name:12} "
              f"{element.element_type.value:18} {element.energy_profile.total_quanta:10,d} "
    ...
    # Детальный энергетический отчет для ключевых элементов
    print(f"\n{'='*80}")
    print("ДЕТАЛЬНЫЙ ЭНЕРГЕТИЧЕСКИЙ ОТЧЕТ ДЛЯ КЛЮЧЕВЫХ ЭЛЕМЕНТОВ")
    print(f"{'='*80}")
    
    key_elements = [1, 2, 6, 8, 10, 11, 17, 18, 26, 29, 47, 79, 82, 86, 92]
    for element in system.elements:
        if element.atomic_number in key_elements:
            element.print_detailed_energy_report()
    
    # Сохранение результатов в файл
    print(f"\nСохранение полных результатов...")
    # Здесь можно добавить код для сохранения в файл
